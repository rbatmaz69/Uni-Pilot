 ...
 *
 * In the desktop app it goes to the ILIAS page and opens the link there, inside
 * Uni Pilot. In a browser tab, where ILIAS cannot be embedded, it opens a tab.
 *
...
export function OpenInIliasButton({ url }: OpenInIliasButtonProps) {
  ...
  const open = async () => {
    setError(null);
    if (canEmbedIlias()) {
      // The link travels in the address, where the ILIAS page picks it up and
      // checks it again before anything opens.
      void navigate(`${NAV_ITEMS.ilias.path}?target=${encodeURIComponent(url)}`);
      return;
    }
    try {
