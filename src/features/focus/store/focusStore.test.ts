...
describe('focus timer', () => {
  ...
  it('starts the break automatically and returns to work after it ends', () => {
    ...
    expect(focus().checkDeadline(61_000)).toMatchObject({
      kind: 'work-completed',
      session: { focusedMs: 60_000, outcome: 'completed' },
    });
    ...
    expect(focus().checkDeadline(181_000)).toEqual({ kind: 'break-completed' });
    expect(focus()).toMatchObject({
      phase: 'work',
      status: 'idle',
      remainingMs: 60_000,
  ...
  it('reconciles delayed background ticks at the original deadline, exactly once', () => {
    ...
    expect(focus().checkDeadline(500_000)).toMatchObject({
      kind: 'work-completed',
      session: { finishedAt: 61_000, focusedMs: 60_000 },
    });
    expect(focus().checkDeadline(600_000)).toEqual({ kind: 'break-completed' });
  ...
  it('restores a running timer and its goal from local storage after a reload', async () => {
    ...
    focus().setBackgroundId('image:forest');
    focus().setMusicId('music:lofi');
    focus().start(1000);
  ...
  it('migrates the former completion-sound preference to ambient audio', async () => {
    localStorage.setItem(
      'uni-pilot.focus',
      JSON.stringify({ state: { soundEnabled: false }, version: 1 }),
    );
    await useFocusStore.persist.rehydrate();
    expect(focus().ambientAudioEnabled).toBe(false);
    expect(focus()).not.toHaveProperty('soundEnabled');
  });
  ...
  it('does not let general configuration or duplicate starts replace an active session', () => {
    focus().start(1000);
  ...
  it('applies edited times to following segments without changing the running segment', () => {
    focus().configureTimes(1, 2);
    focus().start(1000);
    const sessionId = focus().sessionId;

    focus().configureTimes(3, 4);
    expect(focus()).toMatchObject({
      durationMinutes: 3,
      breakMinutes: 4,
      sessionId,
    ...
    expect(focus().checkDeadline(61_000)).toMatchObject({
      kind: 'work-completed',
      session: { plannedMs: 60_000, focusedMs: 60_000 },
    });
    expect(focus()).toMatchObject({
      phase: 'break',
      status: 'running',
      remainingMs: 240_000,
      segmentDurationMs: 240_000,
      deadline: 301_000,
    });

    expect(focus().checkDeadline(301_000)).toEqual({ kind: 'break-completed' });
    expect(focus()).toMatchObject({
      phase: 'work',
      status: 'idle',
      remainingMs: 180_000,
  ...
  it('keeps a paused countdown intact and uses edited times when reset', () => {
    focus().configureTimes(1, 2);
    focus().start(1000);
    focus().pause(21_000);
  ...
  it('only changes the toolbar cover state on request', () => {
    focus().start(1000);
    expect(focus().focusMode).toBe(false);
    focus().setFocusMode(true);
    focus().pause(2000);
    expect(focus()).toMatchObject({ status: 'paused', focusMode: true });
    focus().start(3000);
    expect(focus().focusMode).toBe(true);
  });
});
