...
- **Linux lays out differently.** There Tauri packs all webviews of a window into one vertical GTK box and ignores the positions and sizes set on them. Left alone, the box shares the window evenly: Uni Pilot got the top half and ILIAS the bottom half. The box already stacks them the right way round, so on Linux Uni Pilot is held to the strip's height and ILIAS takes the rest (`stack` in `ilias_view.rs`). The box also sits below the title bar by itself.
